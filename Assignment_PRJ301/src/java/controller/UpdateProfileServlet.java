...
//        processRequest(request, response);
        ...
        if("updateImage".equals(action)){ // check action 
            ...
            if(!PasswordUtil.verifyPassword(oldPassword.toCharArray(), user.getPassword())){
                ...
                String hashed = PasswordUtil.hashPassword(newPassword.toCharArray());
                userDAO.updateUserPass(user.getId(), hashed);
            ...
            userDAO.updateProfileInfo(user.getId(), fullname, email, phone);
