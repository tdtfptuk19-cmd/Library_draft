    ...
    protected void doPostAccess(HttpServletRequest request, HttpServletResponse response, User acc) throws ServletException, IOException {
        ...
        try {
            if ("updateUser".equals(action)) {
                ...
                User selected = userDAO.getUserId(userId);
                if (selected == null) {
                    request.setAttribute("error", "User not found!");
                    request.setAttribute("users", userDAO.getAllUsers());
                    request.getRequestDispatcher("accountManagement.jsp").forward(request, response);
                ...
                String passwordToUse = (password == null || password.isBlank()) ? selected.getPassword() : password;
                User updatedUser = new User(userId, username, passwordToUse, email, fullName, role, selected.getStatus(), selected.getImgUrl(), phone);
